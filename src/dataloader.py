...
def dataset_selector(dataset_name):
    if dataset_name == 'discotube':
        return DiscotubeDataset
    elif dataset_name == 'msd':
        return MSDDataset

def DataLoader(pickle_file, conf, mode='train'):
    logger = logging.getLogger('TrainManager.dataloader')
    ...
    if mode == 'train':
        ...
        dataset = dataset_selector(conf.dataset_name)(pickle_file=pickle_file,
            root=conf.root,
            conf=conf,
        ...
        logger.info(f'number of validation tracks per rank: {conf.tracks_per_rank}')

    logger.info(f'number of samples per process: {len(indices)}')
